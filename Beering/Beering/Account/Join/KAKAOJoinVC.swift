...
class KAKAOJoinVC: UIViewController {
    ...
    func nikDupliCheckBtnInit(){
        ...
        nikDupliCheckBtn.layer.backgroundColor = BeeringColor.Gray01.cgColor
    }
    ...
    @objc func changeUnderLine(){
        if self.nikTextField.text!.count > 0{
            self.nikNameUnderLine.backgroundColor = BeeringColor.Black
        }
        else{
            self.nikNameUnderLine.backgroundColor = BeeringColor.Gray03
        }
    }
    @objc func checkRegEx(){
        regExCondition.text = "영문/한글/숫자✓"
        characterLimit.isHidden = false
        nikTextField.textColor = BeeringColor.Black
        ...
        if checkNikNameRegEx(inputNikName: nikName){
            isinRegEx = true
            self.regExCondition.textColor = BeeringColor.Black
        }
        else{
            isinRegEx = false
            self.regExCondition.textColor = BeeringColor.Gray03
        }
        if nikName.count >= 1 && nikName.count <= 10{
            charLimit = true
            self.characterLimit.textColor = BeeringColor.Black
        }
        else {
            charLimit = false
            self.characterLimit.textColor = BeeringColor.Gray03
        }
    ...
    @IBAction func nikDuplicateBtnTap(_ seder: Any){
        ...
        if validNikName{
            ...
            if isDuplicateNik{
                nikNameUnderLine.backgroundColor = BeeringColor.Green
                regExCondition.textColor = BeeringColor.Green
                regExCondition.text = "사용할 수 있는 닉네임이에요"
            }
            else{
                nikNameUnderLine.backgroundColor = BeeringColor.Red
                regExCondition.textColor = BeeringColor.Red
